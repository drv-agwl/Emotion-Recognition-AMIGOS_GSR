...
def CDASCR(data,fs,min_amplitude):
	CDAdriver = getResult(raw_vector=data, result_type='phasicdriver', sampling_rate=128, downsample=int(128/fs), optimisation=1, pipeout=None)
	CDAphasic = getResult(raw_vector=data, result_type='phasicdata', sampling_rate=128, downsample=int(128/fs), optimisation=1, pipeout=None)
	onsets, pks, amps = find_zeropeak(CDAdriver, min_amplitude, 'CDA',fs)
...
def CVXSCR(data,fs,min_amplitude):
	...
	onsets, pks, amps = find_zeropeak(phasic, min_amplitude, 'CVX',fs)
...
def detrendSCR(data,fs,min_amplitude):
	...
	onsets, pks, amps = find_zeropeak(gsr_detrend,min_amplitude,'detrend',fs)
...
def windowSCR(data,fs,min_amplitude):
	...
	onsets, pks, amps = find_zeropeak(gsr_scr,min_amplitude,'window',fs)
...
def diffSCR(data,fs,min_amplitude):
	...
	onsets, pks, amps = find_zeropeak(df,min_amplitude,'diff',fs)
...
def freqSCR(data,feq,type_filter,fs,min_amplitude):
	if type_filter == 'low':
		...
		onsets, pks, amps = find_zeropeak(gsr_filter,min_amplitude,'filter',fs)
...
def find_zeropeak(data,min_amplitude,task,fs):	
	scrs, amps, ZC, pks = [], [], [], []
	if task=='diff':
	...
	risingtimes = risingtimes/fs
	recovtimes = np.insert(onsets[1:],len(onsets[1:]),len(data))-pks
	recovtimes = recovtimes/fs
	boolean = np.logical_and(risingtimes > 0.1,recovtimes > 0.1)
	pks = pks[boolean]
	onsets = onsets[boolean]
	amps = amps[boolean]
