...
def calculateP(name, label_all, data_all,task):
	...
	for i in range(data_all.shape[1]):
		...
		p1.append(p_1)
		p2.append(p_2)
	file = open('gsr_result/gsr_fusion/cut/ft/'+task+'_pv.csv','w')
...
def main():
	...
	out = [x +'.csv' for x in out]
	for i in range(1,len(task)):
		sumdata = sum(task[0:i])
		trainTask([sumdata,sumdata+task[i]],out[i-1])
	'''
	trainTask([0,2694],'fusion.csv')
	'''
	ori = ['ori1.csv','ori2.csv','ori3.csv']
	for i in range(3):
		sumdata = sum(task[0:1])
		trainTask([sumdata,sumdata+task[1]],ori[i])
	'''
def trainTask(task,outfile):
	...
	writefeat('gsr_result/gsr_fusion/cut/ft/'+outfile,a_best_feature,vote_a,'arousal','w')
	writefeat('gsr_result/gsr_fusion/cut/ft/'+outfile,v_best_feature,vote_v,'valence','a')
