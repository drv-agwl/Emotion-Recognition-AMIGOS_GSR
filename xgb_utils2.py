...
v_feature = {}
collectnum = 5
dir_feature_importances_img = 'img/feature_importance/'

def getbestfeature(task):
	bestfeature = []
	if task == 'arousal':
...
def allfeature_importance(subject,sort_importance,importance,task):
	if task=='arousal':
		for i in range(collectnum):
			find = np.where(importance==sort_importance[i])[0]
			for j in find:
				if j not in a_feature:
					a_feature[j]=[subject]
				else:
					a_feature[j].append(subject)
	
	elif task == 'valence':
		for i in range(collectnum):
			find = np.where(importance==sort_importance[i])[0]
			for j in find:
				if j not in v_feature:
					v_feature[j]=[subject]
				else:
					v_feature[j].append(subject)	


def train(X_train, Y_train, X_val, Y_val, subject, threshold, task):
	...
	fig.savefig(dir_feature_importances_img+task+str(subject)+'.png')
	plt.close()
	...
	allfeature_importance(subject, importance, xgb1result.feature_importances_, task)
	return train_acc, train_f1, val_acc, val_f1, val_roc,feature_size
